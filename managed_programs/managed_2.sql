...
-- ====================================================================================================================================
-- ====================================================================================================================================
--  Users in more than 1 section
-- ====================================================================================================================================

create table users_in_more_than_one_sections AS 
WITH
dat AS (
select 
all_org.statecode      AS  "state_code",
...
AND section_refid IN (
  select 
  sectionrefid 
  from "staging_prod"."v_sections_users" sec_usr
  where usertype != 'students'
  AND userrefid NOT IN 
  ( 
    select userid 
    from "prod_analysis"."ubd_user_program_data" ubd_teacher 
    where ubd_teacher.roles = 'Instructor'   
    )
 )
),
final_dat AS (
...
-- ============================================================================================
-- ============================================================================================
INSERT INTO "prod_analysis"."ubd_user_program_data"
WITH 
eve_prog AS (
select 
  userid,
...
from "staging_prod"."ubd_events" 
where ( actor.roles = array['Learner'] OR actor.roles = array['Instructor'] )
AND actor.districtpid NOT LIKE '91%'
AND actor.districtpid NOT LIKE '92%'
AND actor.districtpid NOT LIKE '79%'
AND actor.districtpid NOT LIKE '88%'
...
CREATE TABLE "prod_analysis"."ubd_user_program_data" AS
WITH 
eve_prog AS (
select 
  userid,
